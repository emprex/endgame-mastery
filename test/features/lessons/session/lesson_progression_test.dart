...
void main() {
  ...
  group('LessonProgression', () {
    test('rejects an empty curriculum', () {
      expect(() => LessonProgression(const <LessonDefinition>[]), throwsArgumentError);
    });

    test('rejects duplicate lesson IDs', () {
      final first = testLesson('duplicate-id');
      final second = testLesson('duplicate-id');
      expect(() => LessonProgression(<LessonDefinition>[first, second]), throwsArgumentError);
    });
    ...
    test('lesson 2 advances to lesson 3', () {
      final progression = LessonProgression(curriculum);
      final session = completedSessionFor(keySquaresLesson02);
      expect(progression.hasNextLesson(session), isTrue);
      expect(progression.nextLessonFor(session), same(keySquaresLesson03));
    });

    test('current final curriculum lesson has no next lesson', () {
      final progression = LessonProgression(curriculum);
      final session = completedSessionFor(pawnTragicomedyLesson06);
